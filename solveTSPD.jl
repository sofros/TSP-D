...
function calculTempsOperation(i::Int, j::Int, k::Int, vDrone::Float64, vCamion::Float64, distancier::Array{Float64,2}, ordre::Array{Int64,1})
    ...
    if k == 0
        ...
        ##println("vCamion: ", vCamion, "   vDrone: ", vDrone)
        for l in i+1:j
...
function synthèse(B, P, M, ordre, vDrone, vCamion, A, f, distancier)
    println("\n============================== Synthèse ==============================")
    println("==  Instace: ", f, " ==\n")
    ...
    k = P[i,j]
    if k == 0
        println("(", ordre[i], ", ", ordre[j], ", ", 0, ") Temps: ", M[i,j])
    else
        println("(", ordre[i], ", ", ordre[j], ", ", ordre[k], ") Temps: ", M[i,j])
    end
...
doublecenter-51-n10.txt:
([0.0, 15.522903717109665, 20.811370455285953, 61.61625878396454, 166.40801216568389, 261.42998517738727, 217.1881835577439, 235.13102257204412, 247.20815514487452, 377.1343267724411, 379.22551639222166], [1, 1, 1, 1, 3, 3, 5, 5, 7, 7, 9])
BISdoublecenter-51-n10.txt:
([0.0, 2.093083381187695, 132.01736124734714, 154.00060020326418, 162.03733283447775, 270.1360453910146, 212.81750422653778, 362.9972972209071, 358.4141459369357, 367.05936893906386, 367.55624569721385, 382.6518857278855], [1, 1, 1, 1, 3, 3, 5, 5, 7, 7, 9, 9])
TRIdoublecenter-51-n10.txt:
([0.0, 15.522903717109665, 15.592516788821618, 24.237739790949753, 62.04949274615442, 169.83438150134768, 264.85635451305103, 220.61455289340773, 238.5573919077079, 250.63452448053835, 373.6740512624135, 380.56069610810493, 382.65377948929256], [1, 1, 1, 1, 3, 4, 3, 6, 6, 8, 8, 10, 10])
=#
